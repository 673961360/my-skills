# skill-evolution-review Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 创建一个手动触发的 Skill，用于评估自有 Skills 体系与公开社区模式的差距，输出结构化改进建议。

**Architecture:** 单个 SKILL.md 文件，定义 4 步工作流（读取本地 → 深度搜索 → 对比分析 → 输出报告）。无额外脚本或 Supporting Files。

**Tech Stack:** Claude Code Skill 标准格式（SKILL.md frontmatter + 正文指令）
...
**Files:**
- Create: `skills/skill-evolution-review/SKILL.md`
...
运行：在 Claude Code 中打开此仓库，确认 `skills/skill-evolution-review/SKILL.md` 的 frontmatter 格式正确：
- `name` 与目录名一致（`skill-evolution-review`）
...
```powershell
.\deploy.ps1 -Skill skill-evolution-review -Target claude-code
```

预期输出：`[DEPLOY] skill-evolution-review -> claude-code`

- [ ] **Step 4: 提交**

```bash
git add skills/skill-evolution-review/SKILL.md
git commit -m "feat: add skill-evolution-review for periodic skills system evaluation"
```
