...
- **激进合并**：同一项目下，将围绕同一主题的多个提交/改动合并为**一条**成果。例如：修复 protobuf 命名空间（1条）+ 修头文件引用（5条）+ 修 Makefile 路径（3条）= 合并为"修复 XX 系列服务的编译链路问题，解决 protobuf 命名空间、头文件引用和路径配置等编译问题"
- **每个项目不超过 4 条成果**：如果某个项目提交很多，必须合并控制在 2-4 条内，宁可少也不要多
- **过滤技术细节**：**不要**在成果条目中出现具体文件名、变量名、函数名、目录路径等代码级细节（如 `write_gtz_*.cpp`、`DeployBase.cs`、`atomic=True`、`grep -vx`）。只保留功能级描述
- **区分成果 vs 过程**：写"完成了 XX"而不是"做了 XX 事"。如"完成 job-schedule 调度框架优化"（成果），不是"优化了 job-schedule 的写入策略"（过程）
...
正确示例（贴近人工撰写的汇报风格）：
- 完成 AI 协作文档体系重构，沉淀上下文协议、领域文档及仓库级导航文档，提升协作效率；
- 完善 SVN 工作流和 AI 编程协作规范，提升复杂 C++ 项目的可维护性和协作效率；
- 完成柜台债独立 SO 模块建设，支持独立编译、组件注册和后续接口扩展；
- 修复 GTZ 系列服务和 Java 部署脚本的编译链路问题，解决 protobuf 命名空间、头文件引用和 cert 目录误删等编译问题；
- 完成产品估值表查询接口接入 API 网关，并同步完善配套文档；
- 优化定时任务调度框架，提升数据同步任务的性能、稳定性和文件写入可靠性。

错误示例（避免）：
- ~~本周进行了 3 次 git 提交~~（过程数据，不是成果）
- ~~修复 write_gtz_quote_cancel_leak_qry_service.cpp 的 missing includes~~（过于技术细节，出现文件名）
- ~~write_csv 增加 atomic=True 默认参数~~（代码级细节，应归并为"完成 XX 框架优化"）
- ~~通过 grep -vx 精确过滤 cjbankupgtz~~（过程描述，不是成果）
- ~~赋能团队协作文档体系~~（空洞词汇）
...
- 项目分组标题后用中文冒号：`C端交易系统：`
- 区块之间空一行
- **每个项目成果控制在 2-4 条**，超过必须合并

#### 下周计划生成规则

基于本周各项目进度，推断下周合理的延续方向：

- **每个项目最多 1 条下周计划**，只保留最自然的延续方向，不要猜测多条
- **延续性**：本周"完成开发"→ 下周"测试上线/联调"；本周"调研分析"→ 下周"提炼规范/模板"；本周"部署接入"→ 下周"能力验证"
- **保守推断**：不编造项目中不存在的功能方向，只基于本周实际工作推断自然延续。如果本周只是搭好了环境，下周计划应该是"验证环境稳定性"而不是"推进大规模使用"
- **不猜测不确定的事**：如果无法从本周数据中判断下周方向，宁可写"继续推进 XX 相关工作"也不要编造具体方向
