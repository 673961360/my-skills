...
| `cat_sql_trade_0013` | 资金事件日历（OMO/发行到期/政府债缴款/一级发行） | 资金市场(04)、一级市场、市场预测 |
...
| `融资回购` / `融券回购` | 回购交易（正/逆） | 交易所业务、银行间业务 |
| `买入` / `卖出` | 个股交易 | 仅交易所业务（权益类） |

> 「债券买入/卖出」≠「买入/卖出」。分组统计时保留完整名称，不可简写。
> 0019 实际委托方向取值带「回购」二字（`融资回购`/`融券回购`），不可简写为「融资/融券」（否则匹配不到任何记录）。
> 方向语义：`融资回购`=正回购=融入资金→「买入」；`融券回购`=逆回购=融出资金→「卖出」。
> 资金类（回购）汇总仅统计 `市场=银行间`，对齐交易员手工口径；交易所回购不计入。
