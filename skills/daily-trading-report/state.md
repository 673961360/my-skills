...
| `04 资金市场分析` | 已渲染 | section、子标题和降级分支已按合同显示；但正式报告中公开市场操作/债券发行与到期未取得真实数据，资金面短评内容与参考图的信息密度仍有差距 |
...
- 现券/资金/一级市场虽然有 QT 短评输入，但当前规则摘要偏机械，未达到“整合多人观点形成市场分析”的目标。

## 下一步

- 先推进 `04 资金市场分析` 的真实内容单元：用 QT 资金面短评和资金事件字段形成更接近参考图的成段分析；API 403 时明确保留数据缺口，不再把占位视为已验证。
- 然后推进 `现券市场分析`：从 QT 现券短评中去重、抽取代表性观点，形成更像参考图的市场研判段落，而不是只列样本消息。
